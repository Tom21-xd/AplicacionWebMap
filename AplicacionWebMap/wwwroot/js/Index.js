...
function calculateRoute() {
    if (stopCoords.length >= 2) {
        ...
        routingControl.on('routesfound', function (e) {
            ...
            summary.classList.add('mb-4', 'font-semibold','text-center');
            const totalDistance = (route.summary.totalDistance / 1000).toFixed(1); // Distancia con un decimal
            const totalTime = route.summary.totalTime;
            const hours = Math.floor(totalTime / 3600); // Convertir segundos a horas
            const minutes = Math.round((totalTime % 3600) / 60); // Calcular los minutos restantes

            summary.innerHTML = `${totalDistance} km, ${hours} h ${minutes} min`;
            directionsPanel.appendChild(summary);

            route.instructions.forEach((instruction, index) => {
                const instructionElement = document.createElement('div');
                instructionElement.classList.add('mb-2', 'text-sm', 'flex', 'items-center');

                const icon = document.createElement('span');
                icon.classList.add('mr-2');
                icon.innerHTML = getDirectionIcon(instruction.type); 
                instructionElement.appendChild(icon);

                const text = document.createElement('span');
                text.innerHTML = `${instruction.text} <span class="text-gray-500 ml-2">${instruction.distance} m</span>`;
                instructionElement.appendChild(text);
...
function removeStop(stopId) {
    const stopElement = document.getElementById(stopId).parentElement; 
    stopElement.remove(); 
...
function clearRoute() {
    ...
    document.getElementById('routeResult').textContent = "Cantidad de peajes : --";

    const directionsPanel = document.getElementById('directionsPanel');
    directionsPanel.innerHTML = '<p class="mt-4 text-center text-gray-500">Su ruta aparecerá aquí</p>';
}
/*
﻿let tollMarkers = []; // Lista para almacenar los peajes
